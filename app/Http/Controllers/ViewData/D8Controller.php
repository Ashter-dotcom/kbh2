...
class D8Controller extends Controller
{
    ...
    public function Index()
    {
        if(!empty(request()->apm) && !empty(request()->model) && !empty(request()->supplier)) {
            ...
            $this->data['results'] = $this->d8Data($this->modelRepository, $this->supplierRepository, $this->componentSupplierInterface, $this->periodeInterface);
        }
        return view('ViewData.D8.Index', ['data' => $this->data]);
    }
    ...
    public function uhduh(Request $request)
    {
        ...
        return view('ViewData.D8.Lihat', compact("apm"));
    }

    public function d8data($model, $supplier, $componentSuppliers, $periode)
    {
        $totals = 1;
    ...
    public function unduh()
    {
        ...
        if(!empty(request()->apm) && !empty(request()->model) && !empty(request()->supplier)) {
            ...
            $results = $this->d8Data($this->modelRepository, $this->supplierRepository, $this->componentSupplierInterface, $this->periodeInterface);

            return Excel::download(new D8Export($results, $bulan, $dataKaryawan, $apm, $model, $supplier), 'DataD8'.$apm->slug.date('Y-m-dH:i:s').'.xlsx');
        }
    ...
    public function getDataStock($stock, $produksi, $delivery)
    {
        ...
        $data['Februari']['2021'] = $stockFebruari;
