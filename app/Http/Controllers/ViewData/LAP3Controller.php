...
class LAP3Controller extends Controller
{
    ...
    public function Index()
    {
        if(!empty(request()->apm) && !empty(request()->model) && !empty(request()->supplier)) {
            ...
            $this->data['results'] = $this->lap3Data($this->modelRepository, $this->supplierRepository, $this->componentSupplierInterface, $this->periodeInterface);
        }
        return view('ViewData.LAP3.Index', ['data' => $this->data]);
    }
    ...
    public function uhduh(Request $request)
    {
        ...
        return view('ViewData.LAP3.Lihat', compact("apm"));
    }

    public function lap3data($model, $supplier, $componentSuppliers, $periode)
    {
        $totals = 1;
    ...
    public function unduh()
    {
        ...
        if(!empty(request()->apm) && !empty(request()->model) && !empty(request()->supplier)) {
            ...
            $results = $this->lap3Data($this->modelRepository, $this->supplierRepository, $this->componentSupplierInterface, $this->periodeInterface);

            return Excel::download(new LAP3Export($results, $bulan, $dataKaryawan, $apm, $model, $supplier), 'DataLAP3'.$apm->slug.date('Y-m-dH:i:s').'.xlsx');
        }
