...
class H11Controller extends Controller
{
    ...
    public function Index()
    {
        if(!empty(request()->apm) && !empty(request()->model) && !empty(request()->periode)) {
            ...
            $this->data['results'] = $this->h11data($this->apmRepository, $this->periodeRepository, $this->komponenRepository, $this->supplierRepository);
        }

        return view('ViewData.H11.Index', ['data' => $this->data]);
    }
    ...
    public function unduh(Request $request)
    {
        if(!empty(request()->apm) && !empty(request()->model) && !empty(request()->periode)) {
            ...
            $results = $this->h11data($this->apmRepository, $this->periodeRepository, $this->komponenRepository, $this->supplierRepository);

            return Excel::download(new H11Export($results, $apm, $periode, $model), 'DataH11'.$apm->slug.date('Y-m-dH:i:s').'.xlsx');
        }

        return  false;
    }

    public function h11data($apm, $periode, $komponen, $supplier)
    {
