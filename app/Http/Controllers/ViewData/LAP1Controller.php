...
class LAP1Controller extends Controller
{
    ...
    public function Index()
    {
        if(!empty(request()->apm) && !empty(request()->model) && !empty(request()->supplier)) {
            ...
            $this->data['results'] = $this->lap1Data($this->modelRepository, $this->supplierRepository, $this->componentSupplierInterface, $this->periodeInterface);
        }
        return view('ViewData.LAP1.Index', ['data' => $this->data]);
    }
    ...
    public function uhduh(Request $request)
    {
        ...
        return view('ViewData.LAP1.Lihat', compact("apm"));
    }

    public function lap1data($model, $supplier, $componentSuppliers, $periode)
    {
        $totals = 1;
    ...
    public function unduh()
    {
        ...
        if(!empty(request()->apm) && !empty(request()->model) && !empty(request()->supplier)) {
            ...
            $results = $this->lap1Data($this->modelRepository, $this->supplierRepository, $this->componentSupplierInterface, $this->periodeInterface);

            return Excel::download(new LAP1Export($results, $bulan, $dataKaryawan, $apm, $model, $supplier), 'DataLAP1'.$apm->slug.date('Y-m-dH:i:s').'.xlsx');
        }
