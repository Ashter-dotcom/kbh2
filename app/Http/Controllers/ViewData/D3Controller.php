...
class D3Controller extends Controller
{
    ...
    public function Index()
    {
        if(!empty(request()->apm) && !empty(request()->model) && !empty(request()->periode)) {
            ...
            $this->data['results'] = $this->d3data($this->apmRepository, $this->periodeRepository, $this->komponenRepository, $this->supplierRepository);
        }

        return view('ViewData.D3.Index', ['data' => $this->data]);
    }
    ...
    public function unduh(Request $request)
    {
        if(!empty(request()->apm) && !empty(request()->model) && !empty(request()->periode)) {
            ...
            $results = $this->d3data($this->apmRepository, $this->periodeRepository, $this->komponenRepository, $this->supplierRepository);

            return Excel::download(new D3Export($results, $apm, $periode, $model), 'DataD3'.$apm->slug.date('Y-m-dH:i:s').'.xlsx');
        }

        return  false;
    }

    public function d3data($apm, $periode, $komponen, $supplier)
    {
