...
class D7Controller extends Controller
{
    ...
    public function d7aIndex()
    {
        ...
        if(!empty(request()->apm) && !empty(request()->periode)) {
            ...
            $this->data['results'] = $this->d7adata($this->apmRepository, $this->periodeRepository);
        }

        return view('ViewData.D7A.Index', ['data' => $this->data]);
    }
    ...
    public function d7aunduh()
    {
        ...
        if(!empty(request()->apm) && !empty(request()->periode)) {
            ...
            return $this->d7adataunduh($this->apmRepository,  $this->periodeRepository);

            return Excel::download(new D7AExport($results, $apm, $periode), 'Data7A'.$apm->slug.date('Y-m-dH:i:s').'.xlsx');
        }
    ...
    public function d7adata($apm, $periode)
    {
        $data = [];
    ...
    public function d7adataunduh($apm, $periode)
    {

        $writer = WriterEntityFactory::createXLSXWriter();
        $filePath = storage_path('excel/d7a_'.date('Y-m-d').'.xlsx');
        $oldFile = storage_path('excel/d7a_'.date('Y-m-d', strtotime('-1 day')).'.xlsx');
