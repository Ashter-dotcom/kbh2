    ...
    <form action="{{ route('user.storeuser') }}" method="POST">
        ...
        <div class="form-row">
            <div class="form-group col-md-4 col-xs-12 col-sm-12 col-lg-4">
                ...
                <input type="text" name="name" class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" id="name" placeholder="Nama" value="{{ old('name') }}">
                @error('name')
            ...
            <div class="form-group col-md-12 col-xs-12 col-sm-12 col-lg-12">
                ...
                <select name="role[]" class="form-control {{ $errors->has('role') ? 'is-invalid' : '' }}" multiple>
                    <option value="">-- Select --</option>
                    @foreach($roles as $key => $role)
                        <option value="{{ $role }}" {{ (collect(old('role'))->contains($role) ? "selected" : "") }}>{{ $role }}</option>
                    @endforeach
                </select>
                @error('role')
                    <div class="invalid-feedback">
