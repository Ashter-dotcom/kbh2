...
@section('content')
    ...
    <form action="{{ route('user.updateuser', ['user_id' => encrypt_decrypt($user->id)]) }}" method="POST">
        @csrf
        <div class="form-row">
            ...
            <div class="form-group col-md-12 col-xs-12 col-sm-12 col-lg-12">
                ...
                <select name="role[]" class="form-control {{ $errors->has('role') ? 'is-invalid' : '' }}" multiple>
                    ...
                    @foreach($roles as $role)
                        <option value="{{ $role }}" {{ (collect(old('role', $user->getRoleNames()))->contains($role) ? "selected" : "") }}>{{ $role }}</option>
                    @endforeach
                </select>
                @error('role')
                    <div class="invalid-feedback">
