...
@section('content')
    ...
    <form action="{{ route('role.updaterole', ['role_id' => encrypt_decrypt($role->id)]) }}" method="POST">
        @csrf
        <div class="form-row">
            <div class="form-group col-md-4 col-xs-12 col-sm-12 col-lg-4">
                <input type="text" name="name" class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" id="role" placeholder="Role Name" value="{{ old('name', $role->name) }}">
                @error('name')
                            ...
                            @foreach($permissions as $permission)
                                <div class="col-md-2 col-xs-6 col-sm-6 col-lg-2">
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" name="permission[]"  type="checkbox" id="permission-{{ $permission->id }}" value="{{ $permission->name }}" {{ (collect(old('permission', $roleHavingPermissions))->contains($permission->name) ? "checked" : "") }}> 
