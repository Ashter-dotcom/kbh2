...
@section('content')
    ...
    <form action="{{ route('master-data-merek-update', ['id' => $merek->id]) }}" method="POST">
        @csrf
            ...
            <div class="card-body">
                <div class="form-row">
                    <div class="form-group col-md-6 col-xs-12 col-sm-12 col-lg-6">
                        ...
                        <option value="{{$merek->apm_id}}" selected>{{$merek->masterDataApm->nama_perusahaan_apm}}</option>
                        </select>
                        @error('apm_id')
                    ...
                    <div class="form-group col-md-6 col-xs-12 col-sm-12 col-lg-6">
                        ...
                        <input type="text" name="merek" class="form-control {{ $errors->has('merek') ? 'is-invalid' : '' }}" id="merek" placeholder="Nama Merek Produk" value="{{ old('merek', $merek->merek) }}">
                        @error('merek')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
