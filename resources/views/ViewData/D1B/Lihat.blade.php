...
@section('content')
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item">H1B</li>
            <li class="breadcrumb-item">{{ $apm->nama_perusahaan_apm }}</li>
        </ol>
    </nav>
    @include('component.alert')
    <form action="{{ route('view-data-d1b-lihat') }}" method="GET">
        <div class="container-fluid card shadow">
            <div class="card-body">
                <div class="form-row">
                    <div class="form-group col-md-10 col-xs-12 col-sm-12 col-lg-10">
                        <label for="apm_id">Nama Perusahaan APM</label>
                        <select id="apm_id" name="apm_id" class="form-control {{ $errors->has('apm_id') ? 'is-invalid' : '' }}" value="{{ old('apm_id',$apm->apm_id) }}">
                            <option value="{{$apm->apm_id}}" selected>{{$apm->nama_perusahaan_apm}}</option>
                        </select>
                    </div>
    ...
    <hr />
    @include('ViewData.D1B.Table',[ 'apm' => $apm, 'supplierComponent' => $supplierComponent ])
@endsection
