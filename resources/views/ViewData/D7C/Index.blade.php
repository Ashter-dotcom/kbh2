...
@section('content')
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item">V10</li>
    ...
    <form action="{{ route('view-data-d7c-index') }}" method="GET" class="needs-validation" novalidate>
    ...
    @if(!empty(request()->apm) && !empty(request()->periode))
        @if(!empty($data['results']['data']))
            <div class="title">
                <p>STOCK UNIT PER END {{ strtoupper(date_bahasa($data['periode']->selesai, ['display_hari' => false])) }}</p>
                ...
                <a href="{{ route('view-data-d7c-d7cunduh', ['apm' => request()->apm, 'periode' => request()->periode]) }}" class="btn btn-dark btn-sm float-right btn-unduh"><i class="fa fa-save"></i> Unduh</a>
            </div>
